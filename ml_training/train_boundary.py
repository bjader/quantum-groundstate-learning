...
This script trains models using optimal hyperparameters on all available data
from jz=1.6 to 6.0 without a test set, using only a 5% validation set.
The trained models will be used to predict observables near the phase boundary
(jz=1.1 to 1.4).

Key difference from train.py:
- No test set (test_frac=0.0) - trains on all available data to maximize
  training data for better extrapolation to the phase boundary region
"""
...
def make_or_load_split(in_files, val_frac=0.05, seed=1, split_path=None,
                       jz_values=None):
    ...
    # No test set for full training
    train_idx, val_idx, _ = stratified_split_by_jz(in_files, jz_values, 
                                                    test_frac=0.0, 
                                                    val_frac=val_frac,
                                                    rng=rng)
...
CONFIG = {
    ...
    "ckpt_every": 10,
    "seed": 1,
    "val_frac": 0.05,  # 5% validation set
    "jz_min": 1.6,     # Train on jz >= 1.6
    "jz_max": 6.0,     # Train on jz <= 6.0
}
...
for observable_key in CONFIG["observables"]:
    ...
    # Load dataset with jz_min only (will filter jz_max manually)
    dataset_all = HeisenbergDataset(root_dir=data_root, n_inputs=1, observable_key=observable_key,
                                    jz_min=CONFIG["jz_min"])
    
    # Filter files to only include jz <= jz_max
    all_files_unfiltered = dataset_all.filenames()
    all_in_files = [f for f in all_files_unfiltered
                    if dataset_all.jz_values[f] <= CONFIG["jz_max"]]
    
    # Update jz_values dict to only include filtered files
...
print("Models saved in model_full/ directory")
print("=" * 80)
